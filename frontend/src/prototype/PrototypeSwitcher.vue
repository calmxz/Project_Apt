<script setup>
// PROTOTYPE - throwaway floating variant switcher. Mounted from ReviewView
// in dev builds only. Cycles ?variant= and mirrors it into sessionStorage so
// the choice survives in-app navigation that drops the query string.
import { computed, onBeforeUnmount, onMounted, watch } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { VARIANTS, fakeData, setVariant, variant } from './recallProto.js'
...
<template>
  <div class="proto-bar" role="group" aria-label="Prototype variant switcher">
    ...
    <span class="proto-bar-label">
      ...
      <em v-if="fakeData">(fake due data; add ?empty=1 for the empty state)</em>
    </span>
