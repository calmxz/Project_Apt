...
export function friendlyError(err) {
  ...
  if (status === 429) return "You've hit the daily limit. Try again tomorrow."
  if (status === 503) return 'The tutor is temporarily unavailable. Try again in a moment.'
  if (typeof status === 'number' && status >= 500) return 'Something went wrong on our side. Try again shortly.'
  if (typeof status === 'number' && status >= 400) return 'That request was rejected. Check the details and try again.'
