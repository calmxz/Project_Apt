...
Collapsed (Ctrl+B, `crux.sidebar.expanded`): a 3rem icon rail, not a bare strip. Top to bottom: the sidebar toggle, New session (drawn plus), Search (drawn lens — clicking it unfolds the sidebar and focuses the search field), Recall (drawn clock with the due count as a small blue numeral when non-zero), then the foot: Settings gear, identity initial. No logo and no per-session markers; sessions appear when the sidebar unfolds.

Footer (both states): Settings as a written line or icon, then the identity row — a 28px initial circle (`card` stock, 1px `card-edge`, blue initial at 700) plus the display name in contents size (folded: the circle alone). The name falls back to the sign-in email's local part (never the full address) when the display name is empty or the user store's unset placeholder. The whole row is one link to Settings; folded, the circle's tooltip is the name. Account and Sign out, once a user menu on this row, sit as text buttons under the Settings title.
