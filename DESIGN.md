...
Learning holds the feedback style picker unchanged, then a Topics overview built from the aggregate profile endpoint: one row per recent topic (topic linking to its session profile, level mark, focus gap underlined in red or "no focus" in pencil, mastered count with a green tick), a small weekly mastery column series, and a concept accuracy list (concept, correct of total, last five results as ticks and crosses in `ink-marker`). Usage is the three usage visuals below. Appearance is unchanged.
