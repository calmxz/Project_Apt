...
### Settings
A tab rail joined to a sheet. Rail tabs: Learning, Usage, Appearance — `desk` fill, `card-edge` on three sides, `6px 6px 0 0`, pencil caption 700, `0.5rem 1rem`; the active tab is `card` in graphite with a -1px bottom margin over the sheet's border. Sheet: `card`, `card-edge`, `0 6px 6px 6px`, hard drop, `1.5rem 2rem 2rem`. Sections are `desk-deep` cards (`card-edge`, 6px, `1rem 1.25rem 1.25rem`, no drop) with a caption 700 graphite title; two columns from 60rem. Submits are `.btn-fill`. The routed page paints `desk-deep` edge to edge. `/settings/profile` redirects to Learning.

Learning holds the feedback style picker unchanged, then a Topics overview built from the aggregate profile endpoint: one row per recent topic (topic linking to its session profile, level mark, focus gap underlined in red or "no focus" in pencil, mastered count with a green tick), a small weekly mastery column series, and a concept accuracy list (concept, correct of total, last five results as ticks and crosses in `ink-marker`). Usage is today's spend against the daily cap: the glance line and the tiered meter. Appearance is unchanged.
...
Collapsed (Ctrl+B, `crux.sidebar.expanded`): a 3rem icon rail, not a bare strip. Top to bottom: the sidebar toggle, New session (drawn plus), Search (drawn lens — clicking it unfolds the sidebar and focuses the search field), Review (drawn clock with the due count as a small blue numeral when non-zero), then one 8px dot per session (pencil at 60%, blue for the current row, `rule-strong` for ended), then the foot: Settings gear, identity initial.

Footer (both states): Settings as a written line or icon, then the identity row — a 28px initial circle (`card` stock, 1px `card-edge`, blue initial at 700) plus the display name in contents size (folded: the circle alone). The name falls back to the sign-in email when the user store's display name is the unset placeholder. Clicking the row opens the user menu: a lifted card in the row-menu grammar with name (700) and email (pencil) as a head block, a rule, then Account and Sign out as one-pitch items; it opens by mouse or keyboard, moves focus in, and closes on Escape, outside click or item choice, returning focus to the row. This replaces the old plain Sign out line.
