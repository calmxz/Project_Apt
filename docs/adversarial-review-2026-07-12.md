...
2. Cost cap SELECT (`:121-137`) → `rate_limit.check_and_increment` (`:140`, **increments before session validation, F-... — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14)**) → session+doc SELECT (`:157-167`, 404 on foreign) → history last-20 (`:177`) → retrieval arbitration (`:205-211`, **sync embedding on the loop, F-18**) → prompt build → user msg commit (`:234-236`).
...
| F-02 | P0 | profile / docs | profile_service.py:343-364 | Flagship focus-clear guard deleted; design doc + CLAUDE.md still claim it is enforced — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
...
| F-07 | P1 | auth | services/auth.py:77 | JWKS/unknown-kid raises uncaught PyJWKClientError → 500 storm; unauth-triggerable JWKS refetch — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-08 | P1 | auth / privacy | frontend/src/stores/user.js:9 | Cross-account leak: user store localStorage key is constant, never cleared on sign-out — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-09 | P1 | frontend | services/apiClient.js:62-66 | No 401 handling anywhere: no refresh-retry, no sign-out, no redirect — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-10 | P1 | agent loop | agent/tutor.py:240-242 | Profile patches bundled with a quiz are silently discarded; model never learns — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-11 | P1 | profile | summary_service.py:44-87 | Summary reloads profile, then overwrites concurrent writes across the LLM call — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-12 | P1 | profile | profile_service.py:145-153 | Whole-blob last-writer-wins profile writes; zero locking; agent ignores ETag — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-13 | P1 | profile | profile_service.py:325-341 | Exclusivity invariant violated by agent+server paths: concept in mastered AND gaps — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
...
| F-20 | P2 | profile | agent/tutor.py:266-270 | Malformed tool-args JSON → empty patch that reports "Profile updated" — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-21 | P2 | profile | profile_service.py:335-341 | Evidence provenance (R4) is LLM-self-reported and gates only review-queue sort order — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-22 | P2 | profile | profile_service.py:165-167 | Case-mismatch leaves focus dangling on a removed gap (exact vs canon compare) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-23 | P2 | profile | profile_service.py:343-349 | Non-focus patch while focus set fails; model's retry silently clears focus — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-24 | P2 | profile | routes/sessions.py:414-420 | Check-answer is 3 commits, no idempotency; double-submit double-counts; crash skips diagnostic grade — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-25 | P2 | profile | diagnostic_service.py:29-30 | Skipping the diagnostic stamps knowledge_level="beginner" permanently, no reset path — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
...
| F-30 | P2 | session | routes/sessions.py:298-307 | Double-end race: two ends both pay an LLM call, doubled spend, nondeterministic summary — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-31 | P2 | session | routes/sessions.py:120-127 | Resume-create finalizes prior without abandoning its open batch → zombie quiz on reopen — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-32 | P2 | session | summary_service.py:54-67 | Ending a 0-message session makes a paid LLM call and stores a hallucinated summary — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-33 | P2 | session | summary_service.py:86-90 | end_session multi-commit around await leaves three distinct partial states, none retried — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
...
| F-36 | P2 | auth | routes/chat.py:140,152-154 | `ensure_user` runs AFTER the FK-bearing usage-counter insert → 500 on first-ever chat — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
| F-37 | P2 | auth | user_service.py:24-33 | `ensure_user` get-then-insert race → IntegrityError 500 on concurrent firsts — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
...
| F-39 | P2 | profile | profile_service.py:311-312 | knowledge_level writable by agent with no evidence; overwrites user-set value on grade — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
| F-40 | P2 | upload | upload.py:58-94 | Full body read before size enforcement when Content-Length absent/spoofed |
| F-41 | P2 | auth | services/auth.py:69-76 | Zero clock-skew tolerance in JWT decode; boundary 401s — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) |
...
| F-48 | P3 | profile | profile_service.py:335-341 | Inferred-mastery add returns ok=True while mutating nothing ("Profile updated" lie) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) |
...
### F-02 — Flagship focus-clear guard deleted; ~~docs still claim it is enforced~~ (P0) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) — guard restored in `apply_patch`; design doc §4.4 + CLAUDE.md §Agent Architecture claims now match shipped behavior
...
### F-07 — JWKS / unknown-kid raises uncaught `PyJWKClientError` → 500 (P1) — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14)
...
### F-08 — Cross-account localStorage leak on a shared browser (P1) — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14)
...
### F-09 — No 401 handling anywhere in the frontend (P1) — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14)
...
### F-10 — Profile patches bundled with a quiz are silently discarded (P1) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15)
...
### F-11 — Summary reloads a stale profile and overwrites concurrent writes (P1) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15)
...
### F-12 — No concurrency control on profile writes; agent ignores ETag (P1) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15)
...
### F-13 — Exclusivity invariant enforced on one of three write paths (P1) — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15)
...
### F-15 — Uploads on ephemeral disk in prod; no persistent mount (P1) — FIXED (Batch 5, fix/adversarial-batch-5, 2026-07-16; fix is the R2 ObjectStore per Q4 — flipping prod to it via `UPLOADS_STORE=r2` + bucket + secrets is an owed deploy gate)
...
**F-39 — knowledge_level writable with no evidence; overwrites user value (profile_service.py:311-312, diagnostic_service.py:31-33).** — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) *Defect:* level changes require no evidence_type (unlike mastery); a user PATCH of level mid-diagnostic-batch is overwritten when the batch resolves. *Trigger:* agent sets level, or user sets level during an open diagnostic. *Consequence:* level set on no evidence, or user intent clobbered. *Confidence:* high. *Fix:* require declared/tested evidence for level changes; grade only if level is still None. *Rejected:* none.

**F-40 — Full body read before size enforcement (upload.py:58-94).** — FIXED (Batch 5, fix/adversarial-batch-5, 2026-07-16) *Defect:* the Content-Length gate is skipped when the header is absent/forged; `file.file.read()` materializes the whole body before the `len(data)` cap. *Trigger:* client omits/forges Content-Length and streams a large body. *Consequence:* resource pressure before rejection. *Confidence:* medium. *Fix:* read in bounded increments, abort past MAX_UPLOAD_BYTES. *Rejected:* trust Content-Length — spoofable.

**F-41 — Zero clock-skew tolerance (auth.py:69-76).** — FIXED (Batch 3, fix/adversarial-batch-3, 2026-07-14) *Defect:* `jwt.decode` has no `leeway`. *Trigger:* backend clock a few seconds ahead of Supabase. *Consequence:* freshly-refreshed tokens 401 at the exp boundary; compounds F-09 into visible failures. *Confidence:* high. *Fix:* `leeway=30`. *Rejected:* none.
...
- **F-48 — Inferred-mastery add returns ok=True no-op (profile_service.py:335-341).** — FIXED (Batch 4, fix/adversarial-batch-4, 2026-07-15) Ignoring inferred mastery is policy; reporting "Profile updated" is a lie. *Fix:* return a distinct status ("inferred mastery ignored").
