...
**Investigation (2026-07-20, Task E8, timeboxed ~45 min):**
- Full live cold-load repro (docker stack, DevTools, throttled/expired-token/backend-down) was **not attempted** — this environment has no running docker stack and no authenticated Supabase test session/credentials available, and the app's router guard redirects any unauthenticated load straight to `/login` before any boot-path API call fires (auth-init failures are caught and only `console.error`'d, never toasted — see `frontend/src/main.js:30-36` and `frontend/src/router/index.js:96-101`), so an "expired token" condition cannot itself reach the calls below. This rules out the auth-init path as a source and narrows the live-repro gap to: needs an already-authenticated session, which was not obtainable here.
- Boot-path API call inventory (every call that can fire during app mount before user interaction), silent/toasted verdict per catch path:
  - `GET /me` via `user.hydrateFromServer()` (`frontend/src/stores/user.js:77-98`, calling `apiGet('/me', undefined, { silent: true })`) — **silent**, explicit opt-out, never toasts.
  - Supabase `getSession()` in `auth.init()` (`frontend/src/stores/auth.js:27-39`, invoked from `frontend/src/main.js:31` and `frontend/src/router/index.js:96-97`) — not routed through `apiClient`/`errorBus` at all; failures only `console.error`'d — **silent**.
  - `GET /sessions` via `store.listSessions()` (`frontend/src/stores/session.js:77-94` calling `sessionsApi.listSessions()` at `frontend/src/services/sessionsApi.js:13`, plain `apiGet('/sessions')` with no `silent` option) fired from `HomeView.vue:86` (`store.listSessions().catch(() => {})`) on every Home mount — **not silent**. The local `.catch(() => {})` only prevents an unhandled-rejection warning; it does not suppress the toast, because `reportApiError()` (`frontend/src/services/apiClient.js:113-114` and `:128-129`) dispatches the `errorBus` `api-error` event *synchronously inside `request()`, before the error is thrown* back to the caller. Confirmed empirically: a scratch vitest (mocked `fetch` rejecting, caller wrapped in `.catch(() => {})`) still recorded exactly one `api-error` event.
  - `GET /review/queue` via `getReviewQueue()` (`frontend/src/services/reviewApi.js:4`, plain `apiGet('/review/queue', params)`, no `silent` option) fired from `HomeView.vue:107-114` (`loadReviewQueue()`, wrapped in try/catch that resets `reviewQueue.value` to empty) on every Home mount — **not silent**, same synchronous-dispatch-before-throw mechanism, same empirical confirmation.
  - `OnboardingView.vue` has no `onMounted` API calls — not part of the boot path.
- All three call sites of `store.listSessions()` in the app (`HomeView.vue:86`, `NewSessionView.vue:164`, `Sidebar.vue:115`) already wrap it in `.catch(() => {})` and either show it inline (`HomeView.vue:6`, `store.error`) or ignore it — none of them want the global toast; same pattern already exists for `skipCheck`/`answerCheck` in `sessionsApi.js:29-32` (F-20: "silent stops the errorBus double-toast").
- `friendlyError()` (`frontend/src/lib/errors.js:16-17`) maps any `status >= 500` (excluding 503, which has its own copy) to the exact string reported in the CONFIRMED sighting: "Something went wrong on our side. Try again shortly." This is the generic-5xx branch, not the `status === 0` ("can't reach the server") branch — consistent with a brief backend 500 on `GET /sessions` or `GET /review/queue` at Home-mount time (e.g. a cold-start race), not a fully-down backend.
- **Conclusion: cause found (mechanism), not a fresh network condition.** `GET /sessions` and `GET /review/queue` are the two candidate sources named in the original finding, and both are demonstrably capable of firing the observed toast text on any transient 5xx, independent of and in addition to each caller's own silent local handling. Root trigger of the transient backend 5xx itself remains unconfirmed (out of scope for a frontend-only, doc-only task) — no live cold-load reproduction was performed end-to-end in a browser in this session; the finding is the code-path proof, not an observed live toast.
- **Fix size: small, same-batch follow-up** (not a new finding) — add `{ silent: true }` to the `apiGet('/sessions')` call in `sessionsApi.listSessions()` and to the `apiGet('/review/queue', params)` call in `reviewApi.getReviewQueue()`, matching the existing F-20 opt-out pattern; both callers already handle failure without the global toast. Not implemented in this task (doc-only, per task constraints).
